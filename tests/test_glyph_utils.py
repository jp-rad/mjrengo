...
from mjrengo.tag_parser import (
    TagParser,
    MARK_LB,
    # MARK_RB,
)
...
class TestGlyphUtilsParseTagContent:
    ...
    def test_parse_simple_glyph(self):
        name, props = TagParser.parse_tag_content("GJ000001")
        assert name == "GJ000001"
        assert props == {}

    def test_parse_glyph_with_props(self):
        name, props = TagParser.parse_tag_content("GJ000001 b=U+30F1 v=U+100000 set=MJ2026")
        assert name == "GJ000001"
        assert props == {"b": "U+30F1", "v": "U+100000", "set": "MJ2026"}

    def test_parse_with_extra_spaces(self):
        name, props = TagParser.parse_tag_content("  GJ000001   b=U+30F1   v=U+100000  ")
        assert name == "GJ000001"
        assert props == {"b": "U+30F1", "v": "U+100000"}

    def test_parse_value_containing_equals(self):
        """値に = が含まれている場合の挙動 (split('=', 1))"""
        name, props = TagParser.parse_tag_content("GJ000001 expr=a=b")
        assert name == "GJ000001"
        assert props == {"expr": "a=b"}

    def test_parse_empty_content(self):
        name, props = TagParser.parse_tag_content("   ")
        assert name == ""
        assert props == {}
...
class TestGlyphUtilsProcessPipeline:
    ...
    def test_pipeline_no_tags(self):
        ...
        result = TagParser.process_pipeline(text, lambda m: m.group(0), unescape=False)
        assert result == "Plain text without any tags."

    def test_pipeline_empty_string(self):
        result = TagParser.process_pipeline("", lambda m: m.group(0), unescape=False)
        assert result == ""
