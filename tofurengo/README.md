...
`tofurengo`（Python版）用の完全な `README.md` です。リポジトリ直下（または `tofurengo` パッケージのルート）にそのまま配置して使用できます。
...
`tofurengo` は、異体字・独自グリフタグ（`{...}`）の正規化（Normalize）および描画（Render）を行うPythonライブラリです。  
...
├── tests/
│   └── test_tofurengo.py      # pytest 用単体テスト
...
```python
from tofurengo import GlyphNormalizer
...
```python
from tofurengo import GlyphRenderer
...
<ElicitationsGroup message="Python配布環境の作成を進める場合、以下のファイル生成が可能です:">
  <Elicitation label="PyPI・pip インストール用の pyproject.toml を作成する" query="tofurengo 用の pyproject.toml を作成してください。"/>
  <Elicitation label="pytest 用のテストファイル（tests/test_tofurengo.py）を作成する" query="tofurengo 用の pytest テストコードを作成してください。"/>
</ElicitationsGroup>
