Module mjrengo.engine
=====================

Functions
---------

`make_replace_fn(glyph_table: Dict[str, Dict[str, Any]], set_name: str) ‑> mjrengo.engine.ReplaceFn`
...
`GlyphResult(success: bool, text: str, errors: List[mjrengo.engine.GlyphError] = <factory>)`
:   GlyphResult(success: bool, text: str, errors: List[mjrengo.engine.GlyphError] = <factory>)

    ### Instance variables

    `errors: List[mjrengo.engine.GlyphError]`
    :   The type of the None singleton.
...
`GlyphTagEngine(replace_fn: mjrengo.engine.ReplaceFn)`
:   Stateless glyph tag processor.
    ...
    `normalize_tags(self, text: str) ‑> mjrengo.engine.GlyphResult`
